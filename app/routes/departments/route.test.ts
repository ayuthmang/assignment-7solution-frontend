...
import { usersByDepartment } from "./route";
import type { UserResponse } from "./type";

describe("usersByDepartment", () => {
  test("summary users by department", () => {
    const { users } = getJsonResponse();
    const slicedUsers = users.slice(0, 2);

    const result = usersByDepartment(slicedUsers);

    expect(result["Engineering"][0]).toEqual(
      expect.objectContaining(slicedUsers[0])
    );
    expect(result["Support"][0]).toEqual(
      expect.objectContaining(slicedUsers[1])
    );
  });
});

function getJsonResponse(): UserResponse {
  return {
    users: [
      {
        id: 1,
